(function($){
	...
	if($("#location-all").length > 0){
		...
		$("#location-all").after('<div id="location-filter"><label>'+objectL10n.filter+': <input type="text" id="filterCats" placeholder="'+objectL10n.type_to_filter+'" /></label> <a class="clear button">'+objectL10n.clear+'</a></div>');
		
		// Cache the elements we use often.
		var $filterCats = $("#filterCats");
		var $categorydiv_selectit = $("#locationdiv .selectit");
		var $categoryFilter_clear = $("#location-filter .clear");
